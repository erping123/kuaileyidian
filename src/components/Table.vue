<template>
  <div class="klyd-table-info">
    <el-card shadow="hover">
      <div class="klyd-table" :style="{background:getTableColor(data.status)}">{{data.tid}}桌号：{{data.status|tableStatus}}</div>
...
export default {
  ...
  methods:{
    ...
    makeQRCode(){
      // 创建二维码——注意此方法不能在当前组件的moubted中调用，因为绘图必须的canvas在el-dialog中，对话框在组件加载时并不在DOM树上
      var qrcode=require('qrcode');
      //var canvas=document.getElementById('qrcanvas');
      // 每个桌子对应的URL应该形如
      //http://127.0.0.1:8091/#/2       2为当前桌子编号
      var tableUrl=this.$store.state.globalSettings.apiUrl+'/#/'+this.data.tid;
      // 把绘制得到的图片二进制数据转换为Base64的字符编码
